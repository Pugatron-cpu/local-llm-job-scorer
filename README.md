...
- `config.MODEL_PRESETS` defines two presets, each pinned to a model, an **Ollama endpoint
  ...
  - `fallback` (`gemma4:12b-it-q8_0`) — the 12B on the **RTX A4000**, served by a *separate*,
    A4000-pinned Ollama instance on `:11435` (see **A4000 fallback endpoint** below); 1 worker,
...
The `fallback` preset points at `http://localhost:11435`, a **second Ollama instance pinned to
...
```ini
[Unit]
Description=Ollama (A4000, port 11435)
...
curl -s 127.0.0.1:11435/api/tags | grep gemma4:12b   # confirm it serves the 12B
```
